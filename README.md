...
- `cowart:cowart-imgae-gen`：旧版兼容入口，保留历史拼写，建议新用法迁移到 `cowart-image-gen`。
- `cowart:cowart-image-edit`：根据用户提供的 Cowart 标注截图生成修订图。

## 项目状态

Cowart 目前仍是早期项目。它适合在可信本地环境中试用和参与改进，但不建议直接暴露到公网。

- 默认只监听 `127.0.0.1:43217`。
- 画布数据写入当前项目的 `canvas/` 目录。
- 安全使用建议见 [SECURITY.md](SECURITY.md)。
- 本地服务健康检查地址：`http://127.0.0.1:43217/api/health`。
- 贡献指南见 [CONTRIBUTING.md](CONTRIBUTING.md)。
- 后续优化方向见 [ROADMAP.md](ROADMAP.md)。
- HTTP API 和 MCP 工具说明见 [docs/API.md](docs/API.md)。
- 原仓库尚未包含开源许可证，复用前请阅读 [LICENSING.md](LICENSING.md)。
